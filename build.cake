...
Task("restore")
    ...
    .Does(() =>
{
    DotNetCoreRestore("./exceldna-abstractions.sln", new DotNetCoreRestoreSettings
    {
        LockedMode = true,
    });
});
...
{
    DotNetCoreBuild("./exceldna-abstractions.sln", new DotNetCoreBuildSettings
    {
...
Task("pack")
    .IsDependentOn("test")
    .DoesForEach(new[] { "ExcelDna.Abstractions", "ExcelDna.IntelliSense.Abstractions", "ExcelDna.Registration.Abstractions" }, (projectName) =>
{
    var releaseNotes = $"https://github.com/augustoproiete/exceldna-abstractions/releases/tag/v{buildVersion.Version}";

    DotNetCorePack($"./src/{projectName}/{projectName}.csproj", new DotNetCorePackSettings
    {
        Configuration = "Release",
        NoRestore = true,
        NoBuild = true,
